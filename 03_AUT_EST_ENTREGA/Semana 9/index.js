...
function centena() {
    ...
    if (num.length < 3 || num.length > 3) {
        window.alert("Deve ter 3 dígitos");
    }
    else {
        var centena = num[0]
        if (centena % 2 === 0) {
            document.querySelector("#result").innerHTML = "Par."
        }
        else {
            document.querySelector("#result").innerHTML = "impar."
        }
    }
...
function sum(){
    ...
    document.querySelector("#sum").innerHTML = "Soma " + sum;
}
...
function fibonacci() {
   ...
   document.querySelector("#series").innerHTML = `Sequência  com ${n} elementos:  ${series}`;
}
...
function prime() {
    ...
    if (val1 > val2) {
        window.alert("Primeiro número deve ser menor");
